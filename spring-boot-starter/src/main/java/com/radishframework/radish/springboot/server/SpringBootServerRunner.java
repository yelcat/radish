...
import com.radishframework.radish.core.registry.ServiceRegistry;
...
import com.radishframework.radish.springboot.RadishProperties.RadishJaegerProperties;
 ...
 */
public class SpringBootServerRunner extends ServerRunnerHelper
  ...
  public void run(String... args) throws Exception {
    ...
    final ServerBuilder<?> serverBuilder = createServerBuilder(serverProperties.getAddress(),
        serverProperties.getPort());

    operationServer = createOperationServer(serverProperties.getOperationPort());
    ...
    globalInterceptors.add(TracingServerInterceptor.newBuilder().withTracer(GlobalTracer.get()).withVerbosity()
        .withStreaming().withTracedAttributes(TracingServerInterceptor.ServerRequestAttribute.PEER_ADDRESS).build());
    ...
    instanceInfo = InstanceInfo.newBuilder().setIp(getHostIp()).setPort(getServerPort())
        .setOperationPort(getOperationPort()).setHostname(getHostName()).setAppName(getAppName()).build();
  ...
  public void onApplicationEvent(@Nonnull ContextClosedEvent event) {
    ...
    if (scheduledExecutor != null) {
      scheduledExecutor = SharedResourceHolder.release(GrpcUtil.TIMER_SERVICE, scheduledExecutor);
    }
  ...
  private void unregisterServices() {
    ...
    scannedServices.forEach(
        serviceDefinition -> getHealthStatusManager().clearStatus(serviceDefinition.getServiceDescriptor().getName()));
  }

  private void startServiceRegister(int interval) {
    if (serviceRegisterTask != null && !serviceRegisterTask.isCancelled() && !serviceRegisterTask.isDone()) {
      serviceRegisterTask.cancel(true);
    }

    serviceRegisterTask = scheduledExecutor.schedule(
        () -> executor.execute(new LogExceptionRunnable(new ServiceRegisterJob())), interval, TimeUnit.SECONDS);
  }
  ...
  private class ServiceRegisterJob implements Runnable {
    ...
    public void run() {
      try {
        scannedServices.forEach(serviceDefinition -> registerService(serviceDefinition.getServiceDescriptor()));
    ...
    private void registerService(@Nonnull final ServiceDescriptor serviceDescriptor) {
      ...
      getLogger().info("Service {} instance {} is registered", serviceName,
